...
void FBXLoader::LoadMesh(FbxMesh* mesh)
{
	...
	const UINT polygonSize = mesh->GetPolygonSize(0);
	assert(polygonSize == 3);

	UINT arrIdx[3];
	UINT vertexCounter = 0; // 정점의 개수

	const UINT triCount = mesh->GetPolygonCount(); // 메쉬의 삼각형 개수를 가져온다
	for (UINT i = 0; i < triCount; i++) // 삼각형의 개수
	{
		for (UINT j = 0; j < 3; j++) // 삼각형은 세 개의 정점으로 구성
		{
