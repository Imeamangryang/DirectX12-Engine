...
void Cube::CreateDescriptorHeap(Graphics* Renderer)
{
	...
	D3D12_SUBRESOURCE_DATA CubeMapData;
	...
	LoadDDSTextureFromFile(Renderer->GetDevice().Get(), L"models/tile.dds", &CubeMap, CubeMapdecodeData, CubeMapData);

	const UINT64 CubeMapSize = GetRequiredIntermediateSize(CubeMap, 0, 1);
	...
	UpdateSubresources(Renderer->GetCommandList().Get(), CubeMap, m_uploadHeap, 0, 0, 1, CubeMapData);

	Renderer->GetCommandList()->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(CubeMap, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE));
...
{
	UINT bufferSize = sizeof(SkyConstantBuffer);
