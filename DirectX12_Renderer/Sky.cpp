	...
	m_orbitCycle(5760)
{
	...
	CreateGeosphere(renderer, 20000, 10);
}
...
void Sky::InitPipeline3D(Graphics* Renderer)
{
    // imgui 초기화
	ImGui_ImplDX12_Init(Renderer->GetDevice().Get(), FRAME_BUFFER_COUNT, DXGI_FORMAT_R8G8B8A8_UNORM,
		m_srvHeap.Get(),
		// You'll need to designate a descriptor from your descriptor heap for Dear ImGui to use internally for its font texture's SRV
		CD3DX12_CPU_DESCRIPTOR_HANDLE(m_srvHeap->GetCPUDescriptorHandleForHeapStart(), 2, m_srvDescSize),
		CD3DX12_GPU_DESCRIPTOR_HANDLE(m_srvHeap->GetGPUDescriptorHandleForHeapStart(), 2, m_srvDescSize));
...
void Sky::CreateConstantBuffer(Graphics* Renderer)
{
	...
	{
		throw (GFX_Exception("Failed to map CBV in Sky."));
	}
}

void Sky::LoadHeightMap(Graphics* Renderer, const wchar_t* displacementmap, const wchar_t* colormap)
{
...
void Sky::CreateGeosphere(Graphics* Renderer, float radius, UINT numSubdivisions)
{
